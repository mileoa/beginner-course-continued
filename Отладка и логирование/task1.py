...
class Robot:
    ...
    def Get_x(self):
        logger.info('Возвращаем значение положения x %s', self.__x)
        return self.__x

    def Get_y(self):
        logger.info('Возвращаем значение положения y %s', self.__y)
        logger.info(self.__y)
        return self.__y

    def Get_temperature(self):
        logger.info('Возвращаем значение температуры %s', self.__temperature)
        logger.info(self.__temperature)
        return self.__temperature

    def Get_noise(self):
        logger.info('Возвращаем значение уровня шума %s', self.__noise)
        return self.__noise

    def Get_health(self):
        logger.info('Возвращаем значение здоровья %s', self.__health)
        return self.__health

    def Set_noise(self, value):
        logger.info('Пытаемся изменить уровень шума робота. Новое значение %s',
                    value)
        if value < 0:
            logger.warning('Значение меньше 0. Операция не будет выполнена.')
            return
        self.__noise = value
        logger.info('Уровень шума установлен %s', self.__noise)
        assert self.__noise >= 0
    
    def Set_temperature(self, value):
        logger.info('Пытаемся изменить температуру робота. Новое значение %s',
                    value)
        ...
        logger.info('Температура робота установлена %s', self.__temperature)
        assert self.__temperature >= 0 and self.__temperature <=1000

    def Set_speed(self, value):
        logger.info('Пытаемся установить новую скорость роботу. Новое значение %s', value)
        if value < 0:
        ...
        logger.info('Скорость робота установлена %s', self.__speed)
    ...
    def Move_to(self, new_x, new_y):
        logger.info('Пытаемся переместить робота на координаты x, y: %s, %s',
                     new_x, new_y)
        if self.__speed == 0:
        ...
        self.__x = new_x
        self.__y = new_y
        logger.info('Робот перемещен. Его координаты x, y: %s, %s',
                    self.__x, self.__y)
        assert self.__x <= 1000 and self.__x >= -1000
        assert self.__y <= 1000 and self.__y >= -1000
  
    def Increase_health(self, inc_value):
        logger.info('Пытаемся увеличить здоровье робота. Текущее значение: %s',
                    self.__health)
        logger.info('Значение увелечения %s', inc_value)
        if inc_value < 0:
        ...
        logger.info('Здоровье робота установлено %s', self.__health)
        assert self.__health > 0 and self.__health <= self.__max_health

    def Decrease_health(self, dec_value):
        logger.info('Пытаемся уменьшить здоровье робота. Текущее значение %s',
                     self.__health)
        logger.info('Знаечние уменьшения %s', dec_value)
...
class Healer(Robot):
    ...
    def Heal(self, target):
        logger.info('Пытаемся выпонить метод лечения. Сила лечения %s',
                    self.__heal_power)
...
class Destroyer(Robot):
    ...
    def Attack(self, target):
        logger.info('Пытаемся выполнить метод атаки цели. Сила атаки %s', 
                    self.__attack_power)
        if isinstance(target, type(None)):
    ...
    def Can_kill_one_shot(self, target):
        ...
        logger.info('Сила атаки %s', self.__attack_power)
        if isinstance(target, type(None)):
            logger.warning('Цель не передана. Операция будет прервана.')
            return
